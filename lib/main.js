...
module.exports = {
  activate() {
    ...
    this.disposables = new CompositeDisposable(
      ...
      // A tab is not inside the view it names, so it needs a scope of its own.
      // Only the active one carries the path the workspace would answer with,
      // which is why the selector says so.
      atom.commands.add(
        "atom-text-editor:not([mini]), .image-view, .image-editor, .pdf-viewer, atom-pane[data-active-item-path] .tab.active",
        {
